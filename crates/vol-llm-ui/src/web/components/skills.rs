...
pub fn SkillsPanel(mut dialog_signal: Signal<SkillDialogState>) -> Element {
    ...
    use_hook(move || {
        ...
        let mut sel = selected_skills_signal.clone();
    ...
    let cap_eff = cap_state.effective_skills.clone();
    let cap_base = cap_state.base_skills.clone();
    let cap_dirt = *cap_dirty.read();
    let cap_has = !cap_state.available_skills.is_empty() || !cap_base.is_empty();
        ...
        div { class: "flex-1 overflow-y-auto p-2.5",
            ...
            div {
                ...
                if cap_loading {
                        ...
                        button {
                            ...
                            onclick: {
                                ...
                                move |_| {
                                    ...
                                    let skills: Vec<String> = sel.read().iter().cloned().collect();
                                    client.agent_update_capabilities(&agent_id, &session_id, vec![], skills, vec![], move |result| {
                                        cs.with_mut(|s| {
                                            match result {
                                                Ok(upd) => { s.effective_skills = upd.effective_skills; s.dirty = false; dirty.set(false); }
                        ...
                        button {
                            ...
                            onclick: {
                                ...
                                move |_| {
                                    let mut sel = sel;
                                    let mut dirty = dirty;
                                    let base = cs.read().base_skills.clone();
                    ...
                    for skill_name in &cap_eff {
                        {
                            let sn = skill_name.clone();
                            let chk = selected_skills_signal.read().contains(&sn);
                            rsx! {
