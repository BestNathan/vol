...
use crate::state::{McpDialogState, McpPromptViewerState};
...
pub fn PromptViewer(mut signal: Signal<McpDialogState>) -> Element {
    ...
    let cached: Rc<RefCell<Option<McpPromptViewerState>>> =
        use_hook(|| Rc::new(RefCell::new(None)));

    let (open, viewer) = {
        let s = signal.read();
        if let Some(v) = &s.prompt_viewer {
            *cached.borrow_mut() = Some(v.clone());
        }
        (s.prompt_viewer.is_some(), cached.borrow().clone())
    };

    rsx! {
        AnimatedOverlay {
            open,
            on_close: move |_| { signal.write_unchecked().prompt_viewer = None; },
