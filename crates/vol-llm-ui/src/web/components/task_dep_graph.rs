...
pub fn build_graph_layout(tasks: &[TaskEntry], center: &str) -> GraphLayout {
    let index: HashMap<&str, &TaskEntry> = tasks.iter().map(|t| (t.id.as_str(), t)).collect();
    ...
    let mut dir_of: HashMap<String, i32> = HashMap::new();
    let mut known_of: HashMap<String, bool> = HashMap::new();
    let mut visited: HashSet<String> = HashSet::new();
    let mut discovery: Vec<String> = Vec::new();

    visited.insert(center.to_string());
    dir_of.insert(center.to_string(), 0);
    known_of.insert(center.to_string(), index.contains_key(center));
    discovery.push(center.to_string());

    // Upstream discovery via `dependencies`.
    let mut up: VecDeque<String> = VecDeque::from([center.to_string()]);
    while let Some(cur) = up.pop_front() {
        if let Some(task) = index.get(cur.as_str()) {
            for dep in &task.dependencies {
                if visited.insert(dep.clone()) {
                    dir_of.insert(dep.clone(), -1);
                    let known = index.contains_key(dep.as_str());
                    known_of.insert(dep.clone(), known);
                    discovery.push(dep.clone());
                    if known {
                        up.push_back(dep.clone());
                    }
                }
            }
        }
    ...
    let mut down: VecDeque<String> = VecDeque::from([center.to_string()]);
    while let Some(cur) = down.pop_front() {
        if let Some(task) = index.get(cur.as_str()) {
            for blk in &task.blocks {
                if visited.insert(blk.clone()) {
                    dir_of.insert(blk.clone(), 1);
                    let known = index.contains_key(blk.as_str());
                    known_of.insert(blk.clone(), known);
                    discovery.push(blk.clone());
                    if known {
                        down.push_back(blk.clone());
                    }
                }
            }
        }
    ...
    let mut layer_of: HashMap<String, i32> = HashMap::new();
    for id in &discovery {
        layer_of.insert(id.clone(), dir_of[id]);
    }
    layer_of.insert(center.to_string(), 0);
            ...
            if let Some(task) = index.get(id.as_str()) {
                ...
                for blk in &task.blocks {
                    if dir_of.get(blk).copied() == Some(1) && here + 1 > layer_of[blk] {
                        layer_of.insert(blk.clone(), here + 1);
                        changed = true;
                    }
                }
                ...
                for dep in &task.dependencies {
                    if dir_of.get(dep).copied() == Some(-1) && here - 1 < layer_of[dep] {
                        layer_of.insert(dep.clone(), here - 1);
                        changed = true;
                    }
                }
            }
    ...
    for id in &discovery {
        if let Some(task) = index.get(id.as_str()) {
            for dep in &task.dependencies {
                if dep != id && layer_of.contains_key(dep) && seen.insert((dep.clone(), id.clone()))
                {
            ...
            for blk in &task.blocks {
                if blk != id && layer_of.contains_key(blk) && seen.insert((id.clone(), blk.clone()))
                {
                    edges.push(GraphEdge {
                        from: id.clone(),
                        to: blk.clone(),
...
pub fn TaskDepGraph(
    ...
    let center_xy: HashMap<String, (i32, i32)> = layout
        .nodes
        .iter()
        .map(|n| {
            let (x, y) = pos(n.layer, n.order);
            (n.id.clone(), (x + NODE_W / 2, y + NODE_H / 2))
        })
        .collect();
        ...
        AnimatedOverlay {
            ...
            div {
                ...
                div { class: "flex items-center justify-between border-b border-[#333355] pb-2 mb-2",
                    div { class: "text-[15px] font-bold text-[#e0e0e0]", "Dependency Graph — {center}" }
                    button {
                        ...
                        for n in layout.nodes.iter() {
                            {
                                ...
                                let nw = NODE_W;
                                let nh = NODE_H;
                                let task = index.get(n.id.as_str());
                                ...
                                let label = if is_center { format!("★ {}", n.id) } else { n.id.clone() };
                                let mut sel = selected;
                                let nid = n.id.clone();
                                rsx! {
                                    g {
                                        style: "cursor: pointer;",
                                        onclick: move |_| sel.set(Some(nid.clone())),
                                        rect {
                        ...
                        rsx! {
                            div { class: "mt-2 pt-2 border-t border-[#333355] text-[12px] text-[#ccc]",
                                if let Some(t) = task {
                                    ...
                                    div { class: "text-[#888]", "{sid} — task not loaded (outside current filter)" }
                                }
                            }
                        }
                    }
...
mod tests {
    ...
    fn t(id: &str, deps: &[&str], blocks: &[&str]) -> TaskEntry {
        TaskEntry {
            id: id.to_string(),
            status: "pending".into(),
            kind: "task".into(),
    ...
    fn has_edge(layout: &GraphLayout, from: &str, to: &str) -> bool {
    ...
    /// Ids render bare: no "t" prefix anywhere in the component source, and the
    /// center node is starred with the raw id.
    ///
    /// The scan is restricted to the source *before* the `#[cfg(test)]`
    /// attribute — otherwise the needles below would match themselves.
    #[test]
    fn ids_render_without_a_t_prefix() {
        let source = include_str!("task_dep_graph.rs");
        let component_src = source
            .split("#[cfg(test)]")
            .next()
            .expect("source before the test module");
        assert!(component_src.contains(r#"format!("★ {}", n.id)"#));
        assert!(!component_src.contains(r#"format!("★ t{}", n.id)"#));
        assert!(!component_src.contains(r#""t{sid}"#));
        assert!(!component_src.contains(r#""t{t.id}"#));
        assert!(!component_src.contains("t{center}"));
    }
