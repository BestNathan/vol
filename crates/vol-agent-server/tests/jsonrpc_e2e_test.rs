...
async fn session_domain_works_with_sqlite_session_store() {
    ...
    let core = match DataPlaneServerCore::builder(temp.path(), temp.path())
        ...
        .await
    {
        ...
        Err(e) => panic!("failed to build DataPlaneServerCore: {e}"),
    };
    ...
    let resp = core
        .handle(AgentServerMessage::new_command(
            ...
            Payload::Session(
                vol_llm_agent_protocol::agent_server_protocol::SessionPayload::List {
    ...
    let Payload::Session(
        vol_llm_agent_protocol::agent_server_protocol::SessionPayload::ListResult { sessions },
    ) = &resp[0].payload
    else {
...
async fn test_e2e_all_methods() {
    ...
    let resp = handle(command(
        ...
        Payload::File(
            vol_llm_agent_protocol::agent_server_protocol::FilePayload::List { path: ".".into() },
        ),
    ))
    .await
    .unwrap();
    ...
    let resp = handle(command(
        ...
        Payload::File(
            vol_llm_agent_protocol::agent_server_protocol::FilePayload::Read {
    ...
    let resp = handle(command(
        ...
        Operation::Session(SessionOperation::List),
        Payload::Session(
            vol_llm_agent_protocol::agent_server_protocol::SessionPayload::List { agent_id: None },
        ),
    ))
    .await
    .unwrap();
    ...
    let resp = handle(command(
        ...
        Payload::Session(
            vol_llm_agent_protocol::agent_server_protocol::SessionPayload::Resume {
                session_id: "n".into(),
                agent_id: None,
            },
        ),
    ...
    let resp = handle(command(
        ...
        Payload::Session(
            vol_llm_agent_protocol::agent_server_protocol::SessionPayload::Entries {
                session_id: "n".into(),
                agent_id: None,
            },
        ),
    ...
    let resp = handle(command(
        ...
        Payload::Log(
            vol_llm_agent_protocol::agent_server_protocol::LogPayload::Read { run_id: "n".into() },
        ),
    ))
    .await
    .unwrap();
    ...
    let resp = handle(command(
        ...
        Payload::Skill(vol_llm_agent_protocol::agent_server_protocol::SkillPayload::List),
    ))
    .await
    .unwrap();
    assert_eq!(resp[0].kind, MessageKind::Result);
    ...
    let resp = handle(command(
        ...
        Payload::Skill(
            vol_llm_agent_protocol::agent_server_protocol::SkillPayload::Get { name: "n".into() },
        ),
    ))
    .await;
    ...
    for (id, op, payload) in [
        (
            ...
            Payload::Mcp(vol_llm_agent_protocol::agent_server_protocol::McpPayload::ListServers),
        ),
        (
            "18",
            Operation::Mcp(McpOperation::ListTools),
            Payload::Mcp(
                vol_llm_agent_protocol::agent_server_protocol::McpPayload::ListTools {
                    server: None,
                },
            ),
        ),
        (
            ...
            Payload::Mcp(
                vol_llm_agent_protocol::agent_server_protocol::McpPayload::ListResources {
                    server: None,
                },
            ),
        ),
        (
            ...
            Payload::Mcp(
                vol_llm_agent_protocol::agent_server_protocol::McpPayload::ListResourceTemplates {
                    server: None,
                },
            ),
        ),
        (
            ...
            Payload::Mcp(
                vol_llm_agent_protocol::agent_server_protocol::McpPayload::ListPrompts {
                    server: None,
                },
            ),
        ),
        (
            ...
            Payload::Mcp(
                vol_llm_agent_protocol::agent_server_protocol::McpPayload::ServerStatus {
                    server: None,
                },
            ),
        ),
