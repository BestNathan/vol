        ...
        match (op, message.payload) {
            ...
            (McpOperation::ListTools, Payload::Mcp(McpPayload::ListTools { server })) => {
                ...
                let tools_json: Vec<serde_json::Value> = tools.iter()
                    .filter(|(s, _)| server.as_ref().is_none_or(|f| s == f))
                    .map(|(s, t)| {
                        serde_json::json!({
            ...
            (McpOperation::Reconnect, Payload::Mcp(McpPayload::Reconnect { server })) => {
                ...
                match mgr.reconnect(&server).await {
                    Ok(()) => {
                        ...
                        tracing::info!(server = %server, status = %status_str, "MCP server reconnected");
                        Ok(vec![AgentServerMessage::new_result(
                            message.message_id,
                            Operation::Mcp(McpOperation::Reconnect),
...
mod tests {
    ...
    #[tokio::test]
    async fn mcp_handler_returns_error_when_not_configured() {
        let handler = McpHandler::new(None);
        let err = handler
            .handle(msg(
                "1",
                Operation::Mcp(McpOperation::ListServers),
                Payload::Mcp(McpPayload::ListServers),
            ))
            .await
            .unwrap_err();
        assert!(err.to_string().contains("mcp not configured"));
    }

    #[tokio::test]
    async fn mcp_handler_rejects_non_mcp_operation() {
        let handler = McpHandler::new(None);
        let err = handler
            .handle(msg(
                "1",
    ...
    #[tokio::test]
    async fn mcp_handler_list_tools_bad_payload_returns_error() {
        let handler = McpHandler::new(None);
        let err = handler
            .handle(msg(
                "1",
            ...
            .await
            .unwrap_err();
        assert!(err.to_string().contains("mcp.list_tools"));
    }

    #[tokio::test]
    async fn mcp_handler_call_tool_bad_payload_returns_error() {
        let handler = McpHandler::new(None);
        let err = handler
            .handle(msg(
                "1",
            ...
            .await
            .unwrap_err();
        assert!(err.to_string().contains("mcp.call_tool"));
    }
}
