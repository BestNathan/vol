...
pub struct ConnectionHolder {
    ...
    receiver: String,
    agent_status:
        Option<Arc<std::sync::RwLock<HashMap<String, crate::data_plane::core::AgentStatus>>>>,
}
...
impl ConnectionHolder {
    ...
    pub fn new(
        ...
        receiver: String,
        agent_status: Option<
            Arc<std::sync::RwLock<HashMap<String, crate::data_plane::core::AgentStatus>>>,
        >,
    ) -> Self {
        Self {
            ...
            match event {
                AgentStreamEvent::AgentStart { input, .. } => {
                    status_map.write().unwrap().insert(
                        self.sender.clone(),
                        crate::data_plane::core::AgentStatus::running(
                            input.clone(),
                            ctx.run_id.clone(),
                        ),
                    );
                ...
                AgentStreamEvent::AgentComplete { .. } | AgentStreamEvent::AgentAborted { .. } => {
                    status_map.write().unwrap().insert(
                        self.sender.clone(),
                        crate::data_plane::core::AgentStatus::idle(),
                    );
                }
                _ => {}
            }
...
mod tests {
    use super::*;
    use vol_llm_agent_protocol::ConnectionError;
    ...
    impl Connection for MockConnection {
        ...
        async fn send(&self, _msg: AgentServerMessage) -> Result<(), ConnectionError> {
            Ok(())
        }
    }
    ...
    async fn test_holder_detach_replaces_connection() {
        let holder = ConnectionHolder::new("sender".to_string(), "receiver".to_string(), None);
        let conn1 = Arc::new(MockConnection {
            protocol: "test1".to_string(),
        });
        let conn2 = Arc::new(MockConnection {
            protocol: "test2".to_string(),
        });
