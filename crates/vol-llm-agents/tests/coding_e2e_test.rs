...
async fn test_coding_agent_e2e_read_file() {
    ...
    let result = agent
        .run("Read the test.txt file and tell me its content")
        .await
        .unwrap();
...
async fn test_coding_agent_e2e_edit_file() {
    ...
    let result = agent
        .run("Change the print output from \"old\" to \"new\" in test.rs")
        .await
        .unwrap();
...
async fn test_coding_agent_html_report_contains_timeline() {
    ...
    assert!(
        content.contains("Agent started"),
        "Report should contain AgentStart event"
    );

    // Should have thinking events
    assert!(
        content.contains("Thinking"),
        "Report should contain ThinkingComplete event"
    );

    // Should have tool call events (read_file)
    assert!(
        content.contains("Tool Call") || content.contains("read_file"),
        "Report should contain ToolCall events"
    );

    // Should have completion event
    assert!(
        content.contains("Agent completed"),
        "Report should contain AgentComplete event"
    );
