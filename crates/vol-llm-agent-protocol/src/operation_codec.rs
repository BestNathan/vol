...
pub fn method_to_operation(method: &str) -> Result<Operation, ProtocolError> {
    match method {
        ...
        "control.register" => Ok(Operation::Control(ControlOperation::Register)),
        "control.heartbeat" => Ok(Operation::Control(ControlOperation::Heartbeat)),
        "control.capability_snapshot" => {
            Ok(Operation::Control(ControlOperation::CapabilitySnapshot))
        }
        "control.capability_delta" => Ok(Operation::Control(ControlOperation::CapabilityDelta)),
        "control.event" => Ok(Operation::Control(ControlOperation::Event)),
        "control.command" => Ok(Operation::Control(ControlOperation::Command)),
        "control.command_ack" => Ok(Operation::Control(ControlOperation::CommandAck)),
        "control.command_result" => Ok(Operation::Control(ControlOperation::CommandResult)),
        "control.node_list" => Ok(Operation::Control(ControlOperation::NodeList)),
