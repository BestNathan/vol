# Docs-RS MCP Integration Example Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a single runnable example (`docs_rs_mcp_example.rs`) that demonstrates the full MCP integration flow — config loading, server connection, tool discovery, and tool execution via `with_mcp_from_config()`, prompting the agent to search for the dioxus crate.

**Architecture:** The example creates a temp directory with a `.mcp.json` file pointing to the `docs-rs-mcp` binary, builds a `ReActAgent` via `AgentConfig::builder().with_mcp_from_config(Some(&tmp_dir))`, then runs it with a Chinese-language prompt to search for dioxus documentation.

**Tech Stack:** Rust, tokio, tempfile, vol-llm-agent (ReActAgent), vol-llm-mcp (McpSession), vol-llm-provider (AnthropicProvider), vol-mcp-servers (docs-rs-mcp binary)

---

### Task 1: Create the example file and verify compilation

**Files:**
- Create: `crates/vol-llm-agents/examples/docs_rs_mcp_example.rs`
- Test: `cargo check --example docs_rs_mcp_example -p vol-llm-agents`

- [ ] **Step 1: Create the example file**

Write the complete example to `crates/vol-llm-agents/examples/docs_rs_mcp_example.rs`:
...
use vol_llm_provider::{LLMConfig, anthropic::AnthropicProvider, Secret};
...
Run:
```bash
cargo check --example docs_rs_mcp_example -p vol-llm-agents
```

Expected: No errors. The example compiles cleanly.

- [ ] **Step 3: Verify the example binary is registered**

Run:
```bash
cargo run --example docs_rs_mcp_example -p vol-llm-agents -- --help 2>&1 | head -5
```

Expected: The binary starts (will fail due to missing env vars, but should not fail at link/parse time).

- [ ] **Step 4: Commit**

```bash
git add crates/vol-llm-agents/examples/docs_rs_mcp_example.rs
git commit -m "feat: add docs-rs MCP integration example

Demonstrates ReActAgent connecting to docs-rs MCP server via
with_mcp_from_config(), searching for the dioxus crate.
