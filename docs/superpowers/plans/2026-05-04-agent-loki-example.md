# K8s Ops Agent with Loki Example

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a runnable example that defines a k8s ops agent in a `.md` file, loads it via `AgentLoader`, runs it with a real LLM against the Anthropic/DashScope API, and sends logs to Loki.

**Architecture:** The example creates a `.agents/agents/k8s_ops_agent.md` file in CWD. An `AgentLoader` discovers it from the `Repo` scope. A `ReActAgent` is built via `AgentConfig::builder()` with the loaded `AgentDef`, a real `AnthropicProvider` (DashScope endpoint), and `LokiPlugin` registered. The agent runs a k8s cluster status query.

**Tech Stack:** Rust, `vol-llm-agent` (AgentLoader, ReActAgent, AgentDef), `vol-llm-observability` (LokiPlugin, LokiConfig), `vol-llm-provider` (AnthropicProvider, LLMConfig, Secret), `tokio`, `tracing-subscriber`

---

### Task 1: Create the example agent definition file

**Files:**
- Create: `crates/vol-llm-agents/examples/k8s_ops_agent.md`

- [ ] **Step 1: Write the agent definition file**

Create `crates/vol-llm-agents/examples/k8s_ops_agent.md` with this content:
...
Run:
```bash
cat crates/vol-llm-agents/examples/k8s_ops_agent.md
```

Expected: Valid YAML frontmatter followed by markdown body.

- [ ] **Step 3: Commit**

```bash
git add crates/vol-llm-agents/examples/k8s_ops_agent.md
git commit -m "feat: add k8s ops agent definition file for example"
```

---

### Task 2: Create the runnable example

**Files:**
- Create: `crates/vol-llm-agents/examples/agent_loki_example.rs`

- [ ] **Step 1: Write the example code**

Create `crates/vol-llm-agents/examples/agent_loki_example.rs` with this full content:
...
Run:
```bash
cargo check -p vol-llm-agents --example agent_loki_example
```

Expected: `Finished dev [unoptimized] target(s)` with no errors.

If `tracing-subscriber` is not available as a dev-dependency, it already is — check `crates/vol-llm-agents/Cargo.toml`:
```toml
[dev-dependencies]
...
git commit -m "feat: add k8s ops agent example with Loki observability"
```
