...
3. **parent / depth 无记录**——嵌套派发时子 agent 继承的是父的注册表（同一 `AgentTool` 实例），现有 `max_depth` 检查实际不随嵌套递增，循环派发（A→B→A）不受控。
...
5. **parent / depth 记录**：子 agent 携带并记录 `parent_agent` 与 `depth`（每次派发 depth+1），写入可观测的位置（会话元数据 / 配置），供后续按 depth 做限制。
6. **循环 / 自派发支持**：允许 A→B→A 或派发给自己的定义，每次派发等价于新开一个 session；仍受 `max_depth` 检查约束（叠加 depth 记录后具备更准确的限制基础）。

## Non-Goals

- **不落实** `AgentDef.tools` / `disallowed_tools` 的工具过滤（维持「子 agent 继承父工具集」现状；overlay 只处理 `agent` 工具本身）。
- **不落实** `AgentDef.model` 模型覆盖（子 agent 继承父 LLM，现状不变）。
...
6. 深度防护：达到 `max_depth` 的派发返回错误（现有单测保留）。
7. 覆盖率：涉及 crate 的 `just cover-gate <crate> 80` 通过。
...
- **循环 / 自派发**：每次派发新开 session；depth 记录递增；达到 max_depth 时报错。
...
2. 子 agent session 的键规则（session_id 生成方式、按 agent type/name 存储）——设计阶段结合 SessionManager 现状确定。
