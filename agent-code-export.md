...
The ReActAgent now uses RunContext for unified state management.

### Key Changes:
1. `RunContext` created at start of `run()` method
2. Contains immutable fields (run_id, user_input, session_id) and mutable fields (messages, tool_calls via RwLock)
3. Plugins use `&RunContext` instead of `&PluginContext`
