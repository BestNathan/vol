...
function EntryView({ entry }: { entry: ConversationEntry }) {
  switch (entry.type) {
    ...
    case 'Thinking':
      return (
        <div className="mb-2.5 px-2.5 py-2 rounded-md max-w-full break-words whitespace-pre-wrap text-[#888] italic text-[12px] leading-[1.5]">
          {entry.content}
        </div>
      )
    case 'ToolCall':
      return (
        <div className="mb-2.5 px-2.5 py-2 rounded-md max-w-full break-words whitespace-pre-wrap text-[#e0e0e0] text-[13px]">
          <span className="text-[#f0c040] font-bold">[tool]</span>{' '}
          <span className="font-semibold">{entry.toolName}</span>{' '}
          <span className="text-[#888]">{entry.argPreview}</span>
        </div>
      )
    case 'ToolResult': {
      ...
      return (
        <div className={cls}>
          ...
          <div className="text-[#888] text-[12px] mt-1 pl-1 max-h-[120px] overflow-y-auto font-mono">
    ...
    case 'EntryCheckpoint': {
      ...
      return (
        <div className="mb-2.5 px-2.5 py-2 rounded-md max-w-full break-words whitespace-pre-wrap bg-[#2a2a20] border-l-[3px] border-[#c0a040] text-[#aaa] text-[12px] italic">
    ...
    case 'RunSummary': {
      ...
      return (
        <div className="mb-2.5 px-2.5 py-2 rounded-md max-w-full break-words whitespace-pre-wrap text-[#80c080] font-bold py-1.5">
    ...
    case 'AgentAnswer':
      return (
        <div className="mb-2.5 px-2.5 py-2 rounded-md max-w-full break-words whitespace-pre-wrap text-[#e0e0e0] leading-[1.5]">
          {entry.text}
        </div>
      )
    default:
    ...
    <Dialog open={open} onOpenChange={(next) => { if (!next) onClose() }}>
      <DialogContent className="sm:max-w-[900px] h-[70vh] max-h-[70vh] flex flex-col gap-0 p-0 overflow-hidden">
        <DialogHeader className="flex flex-row items-center justify-between px-3 py-2 border-b border-[#2a2a44] space-y-0 text-left">
          <DialogTitle className="font-mono text-[13px] text-[#e0e0e0] truncate">
            Session: {session?.id ?? ''}
          </DialogTitle>
        </DialogHeader>
        {loading ? (
          <div className="flex items-center justify-center flex-1 text-[#666]">Loading...</div>
        ) : parseFailure && entries.length === 0 ? (
          <div className="flex-1 flex items-center justify-center flex-col text-[#ff6060] p-5 text-center">
            <div className="text-[14px] font-semibold mb-2">Failed to parse session entries</div>
            <div className="text-[12px] text-[#888]">Check browser console (F12) for details</div>
          </div>
        ) : entries.length === 0 ? (
          <div className="flex items-center justify-center flex-1 text-[#666]">No entries</div>
        ) : (
