...
function t(id: number, deps: number[], blocks: number[]): TaskEntry {
  return {
    id,
    status: 'pending',
    kind: 'task',
...
function node(layout: GraphLayout, id: number) {
  const n = layout.nodes.find((n) => n.id === id)
  expect(n, `node ${id} present`).toBeDefined()
  return n!
}

function hasEdge(layout: GraphLayout, from: number, to: number): boolean {
